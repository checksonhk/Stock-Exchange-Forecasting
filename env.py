...
class TradingEnv(gym.Env):
    ...
    - price is discretized to ints, to reduce state space
    ...
    - when buying, buy as many shares as cash in hand allows
    - if buying multiple stock, equally distribute cash in ahnd and then utilize the balance
    """
    
    def __init__(self, train_data, init_invest=20000):
        
        # data
        self.stock_price_history = np.around(train_data)  # round up ints
    ...
    def _step(self, action):
        ...
        self.current_step += 1
        self.stock_price = self.stock_price_history[:, self.current_step]  # update price
        self._trade(action)
    ...
    def _trade(self, action):
        # all combanations to sell(0), hold(1), buy(1)
        ...
        for i, a in enumerate(action_vec):
            ...
            if a == 2:
                buy_index.append(i)
        # two passes: sell first, then buy; naive in real world settings
        if sell_index:
            for i in sell_index:
            ...
            while can_buy:
                for i in buy_index:
                    if self.cash_in_hand > self.stock_price[i]:
                        self.stock_owned[i] += 1 # buy one more share
