...
if __name__ == "__main__":
    ...
    if args.mode == "test":
        ...
        # when test, the timestap is the same time as when the weights is trained
    ...
    for e in range(args.episode):
        ...
        for time in range(env.n_step):
            ...
            state = next_state
            if done:
                print("episode: {}/{}, episode end value: {}".format(e+1, args.episode, info['current_val']))
                portfolio_value.append(info['current_val'])  # append episode end portfolio value
                break
            if args.mode == "train" and len(agent.memory) > args.batch_size:
                agent.replay(args.batch_size)
        if args.mode == "train" and not (e+1) % 10:  # checkpoint weight
